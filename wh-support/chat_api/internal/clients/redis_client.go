...
const (
	TTLForTicketInfo         = 60 * time.Minute
	TTLForLastChatViewByUser = 24 * time.Hour
)
...
func (r *RedisClient) AddLastChatViewByUser(ctx context.Context, employeeID, ticketID int64, now string) error {
	key := getLastChatViewByUserKey(employeeID, ticketID)

	_, err := r.Set(ctx, key, now, TTLForLastChatViewByUser).Result()
	if err != nil {
		return fmt.Errorf("can't add data to redis: %w", err)
	}

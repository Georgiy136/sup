...
type TicketsScenariosExportCron struct {
	cronCfg cron_models.CronCommonCfg

	kafkaOlapClient *clients.WhKafkaOlapWebPublisherClient
}

func NewTicketsScenariosExportCron(kafkaOlapClient *clients.WhKafkaOlapWebPublisherClient) *TicketsScenariosExportCron {
	return &TicketsScenariosExportCron{
		kafkaOlapClient: kafkaOlapClient,
	}
}

func (s *TicketsScenariosExportCron) Start(ctx context.Context, conf configs.Config, cronConf cron_models.CronCommonCfg) {
	s.cronCfg = cronConf

	s.Work()
}

func (s *TicketsScenariosExportCron) Work() {
	const (
		maxBatchLen = 10
		apiKey      = "support.tickets.scenarios"
		sp          = "sync.scenariosync_exporttojson"
		ackSp       = "sync.scenariosync_delsynced"
	)
	...
	for {
		...
		if err != nil {
			logrus.Errorf("[%v] can't get scenarios from db, err %v, sleep %v", s.cronCfg.CronTaskName, err, s.cronCfg.CronTimeSleepOnErrorParsed)
			time.Sleep(s.cronCfg.CronTimeSleepOnErrorParsed)
			continue
		}

		var ticketsScenariosFromDb models.ResponseTicketsScenariosFromDB

		if err = jsoniter.Unmarshal(dbResponse, &ticketsScenariosFromDb); err != nil {
			logrus.Errorf("[%v] err unmarshal db response, err: %v, sleep %v", s.cronCfg.CronTaskName, err, s.cronCfg.CronTimeSleepOnErrorParsed)
			time.Sleep(s.cronCfg.CronTimeSleepOnErrorParsed)
			continue
		}

		if len(ticketsScenariosFromDb.ScenariosData) == 0 {
		...
		batch := make([]*sync_models.Scenarios_Scenario, 0, maxBatchLen)

		for _, scenarioData := range ticketsScenariosFromDb.ScenariosData {
			dt, err := timeutils.ParseDBTime(scenarioData.ChDt)
			if err != nil {
				logrus.Errorf(
			...
			scenarioData.ChDt = timeutils.ToMoscowRFC3339Nano(dt)

			batch = append(batch, scenarioData)
...
func (s *TicketsScenariosExportCron) sendAndAcknowledgeBatch(batch []*sync_models.Scenarios_Scenario, apiKey, ackSp string) error {
	if len(batch) == 0 {
		return nil
	}
	...
	ticketsScenariosProto := sync_models.Scenarios{Data: batch}
	ticketsScenariosConverted, err := utils.ConvertTicketsScenarios(&ticketsScenariosProto)
	if err != nil {
		logrus.Errorf("[%v] can't convert ticket scenarios to json, log_ids: %v, err: %v, sleep %v", s.cronCfg.CronTaskName, logIDs, err, s.cronCfg.CronTimeSleepOnErrorParsed)
		return err
	}

	if err := s.kafkaOlapClient.SendData(apiKey, ticketsScenariosConverted.Data); err != nil {

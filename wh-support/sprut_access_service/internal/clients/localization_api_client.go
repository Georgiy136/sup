...
func NewLocalizationApiClient() *LocalizationApiClient {
	return &LocalizationApiClient{
		httpClient: client.NewHttpClient(),
	}
}

func (l *LocalizationApiClient) Configure(ctx context.Context, config configs.Config) {
	l.httpClient.InitHttpClient(localizationConfKey)(ctx, config)
}
...
func (l *LocalizationApiClient) GetTranslation(ctx *gin.Context, lang string, keysWithValues models.KeysWithValues) (string, error) {
	...
	if response == nil {
		return "", l.httpClient.GenerateError(response, fmt.Errorf("invalid response: %w", ErrResponseNil), apiKey)
	}
	...
	default:
		return "", l.httpClient.GenerateError(response, fmt.Errorf("unexpected status code %d: %w", response.StatusCode(), ErrWrongStatusCode), apiKey)
	}
}

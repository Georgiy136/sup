...
func (t *TicketNotificationsService) handlePerformOperation(ctx context.Context, notification ticketmodels.TicketNotification, requiredSiteAction bool) error {
	...
	if err != nil {
		return fmt.Errorf("can't send notification message: %w", err)
	}
...
func (t *TicketNotificationsService) handlePerformedOperation(ctx context.Context, notification ticketmodels.TicketNotification) error {
	...
	statusText := templates.GetTicketStatusTextByOperationType(consts.PerformedOperation, notification.TicketInfo.ResponsibleEmployeeID)
	for _, post := range posts {
		if post.Operation != consts.PerformOperation {
			continue
		}
	...
	return t.ticketPostCache.DeleteBandTicketPostsByOperation(ctx, notification.TicketID, channelID, consts.PerformOperation)
}

...
func Test_TreeInfoCreateRejectTicket(t *testing.T) {
	setPort()

	queries := setQueries()

	cute.NewTestBuilder().
		Title("Получение дерева -> информации о категории -> создание -> отклонение заявки").
		Tags("category").
		CreateStep("Получение дерева категорий").
		RequestBuilder(
			cute.WithURI(fmt.Sprintf("http://localhost:%s/api/categories/info", port)),
		...
		AssertBody(
			cute.AssertBody(customAssertTreeBody()),
		).NextTest().
